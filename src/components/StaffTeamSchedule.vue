              ...
              <b-list-group flush>
                <template v-if="dayInfo.requests.length">
                  ...
                  >
                    ...
                    <div>
                      <b-badge :variant="getStatusVariant(request.Status)">{{ request.Status }}</b-badge>
                      <b-badge v-if="request.Comments" variant="info" class="ml-2">
                        {{ request.Comments }}
                      </b-badge>
                    </div>
                  </b-list-group-item>
    ...
    setup() {
      ...
      const previousPeriod = () => {
        if (isMonthView.value) {
          ...
          viewingDate.value = new Date(viewingDate.value.getFullYear(), viewingDate.value.getMonth(), viewingDate.value.getDate() - 7);
        }
      };
